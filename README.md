...
```bash
curl -O https://raw.githubusercontent.com/你的用户名/response-proxy/main/response-proxy.mjs
```
...
> - MiniMax：https://platform.minimaxi.com/document/Apikey
...
# 方式三：使用环境变量预设名
UPSTREAM_BASE_URL=deepseek node response-proxy.mjs

# 智谱 GLM（默认，不需要指定）
node response-proxy.mjs
```

> **可用预设名**：`glm`（智谱常规）、`glmcp`（智谱 Coding Plan）、`deepseek`、`kimi`、`qwen`（通义千问常规）、`qwencp`（百炼 Coding Plan）、`doubao`（豆包常规）、`doubaocp`（方舟 Coding Plan）、`minimax`（MiniMax 常规）、`minimaxcp`（MiniMax Coding Plan）、`ollama`（本地）

看到 `response-proxy v1.0.0 已启动` 就说明启动成功了。
...
选项:
  ...
  --upstream <URL>     上游 Chat Completions 地址
